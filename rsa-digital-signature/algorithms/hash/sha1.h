...
public:
    ...
    virtual BigInt getDigest(std::vector<uint8_t>& data) override;
    
private:
    void preProcessData(std::vector<uint8_t>& data);
    void processBlock(const std::vector<uint8_t>& data, size_t blockOffsetIndex);
    ...
    const int BlockSize_ = 64;      // In uint8_ts
